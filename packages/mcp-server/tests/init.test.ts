...
describe("detectTools", () => {
  ...
  it("detects multiple tools simultaneously", async () => {
    ...
    const claudeCodePath = join("/mock/home", ".claude", "mcp.json");
    ...
    mockedExistsSync.mockImplementation(
      (p) =>
        p === claudeDesktopPath || p === claudeCodePath || p === windsurfPath,
    );
...
describe("writeConfig", () => {
  ...
  it("writes config to an empty/nonexistent file", async () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith("/mock/home/config", {
      recursive: true,
    });
