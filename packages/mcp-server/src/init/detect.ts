...
export async function detectTools(): Promise<DetectedTool[]> {
  ...
  // ── Claude Code ───────────────────────────────────────────────────────────
  // Claude Code reads MCP servers from ~/.claude/mcp.json (NOT settings.json)
  const claudeCodeConfig = join(home, ".claude", "mcp.json");
  ...
  } else if (existsSync(join(home, ".claude"))) {
    // ~/.claude/ exists but mcp.json doesn't — create it
    tools.push({
      name: "Claude Code",
      configPath: claudeCodeConfig,
      type: "json-mcpServers",
